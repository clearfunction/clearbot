...
module.exports = (robot) ->
  ...
  alertNoClients = (res) ->
    msg = "Sorry, I don't have any Sonos relay clients connected right now. You can download one here... #{RELAY_CLIENT_DOWNLOAD_URL}"
    robot.send {room: res.message.user.name}, msg
    setTimeout () ->
      msg = "... Burn! :fire: http://i.imgur.com/4lhFLpO.gif"
      robot.send {room: res.message.user.name}, msg
    , 3 * 1000
